...
type ChatConfig struct {
	...
	User      string    `gorm:"column:user;not null;default:''::character varying;comment:用户标识" json:"user"`       // 用户标识
	AgentID   int64     `gorm:"column:agent_id;not null;comment:应用ID" json:"agent_id"`                             // 应用ID
	Model     string    `gorm:"column:model;not null;default:''::character varying;comment:模型" json:"model"`       // 模型
	Prompt    string    `gorm:"column:prompt;not null;default:''::character varying;comment:系统初始设置" json:"prompt"` // 系统初始设置
